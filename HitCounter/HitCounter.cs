    ...
    public class HitCounter : Mod, IGlobalSettings<GlobalData>, ICustomMenuMod, ITogglableMod
    {
        ...
        private void UpdateData()
        {
            for (var s = 0; s < _currentCounter.splits.Count; s++)
            {
                ...
                if (counter.splits.Count == 0) counter.splits = new List<int>(new int[_currentCounter.splits.Count]);
                counter.splits[s] = split.hitsPb;
            }
            Log($"Saved PBs for {_currentCounter.title}");
        }
        ...
        {
            var menu = NewModMenu.GetMenu(modListMenu, toggle);
            return menu;
        }
